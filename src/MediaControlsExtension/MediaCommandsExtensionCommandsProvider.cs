...
public sealed partial class MediaControlsExtensionCommandsProvider : CommandProvider
{
    ...
    {
        this.Id = "com.dziad.mediacontrolsextension.provider";
        ...
        this._mediaControlsPageItem = new(mediaControlsExtensionPage) { Title = this.DisplayName, Subtitle = "Playback controls, now playing info, and volume", Icon = this.Icon };
        ...
        this._bands = [new CommandItem(mediaControlsBand) { Title = "Media Controls Dock", Subtitle = "Playback, track, session, and volume controls", Icon = this.Icon }];
    ...
    private Task? InitializeFallbackCommands()
    {
        try
        {
            ...
            this._fallbackCommands = [
                new FallbackPlayCommandItem(new PlayPauseMediaCommand(sessionManagerTask!, this._settingsManager, this._yetAnotherHelper), "Toggle Play / Pause", this._settingsManager),
    ...
    private void UpdateTopLevelCommands()
    {
        this._commands = this._settingsManager.ShowCurrentMediaAtTopLevel
            ? [this._mediaControlsPageItem, this._nowPlayingItem]
            : [this._mediaControlsPageItem];
        this.RaiseItemsChanged();
    }
